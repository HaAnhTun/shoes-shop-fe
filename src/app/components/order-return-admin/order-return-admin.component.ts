...
export class OrderReturnAdminComponent implements OnInit {
  ...
  cancelOrderReturn(id: number) {
    this.confirmationService.confirm({
      message: "Bạn muốn hủy yêu cầu đổi trả?",
      header: "Xóa hóa đơn",
  ...
  finish(id: number) {
    this.confirmationService.confirm({
      message: "Bạn muốn câp nhật thông tin yêu cầu?",
      header: "Xóa hóa đơn",
      icon: "pi pi-exclamation-triangle",
      accept: () => {
        this.http
          .get("http://localhost:8088/api/order-returns/finish/" + id)
          .subscribe((res) => {
            console.log(res);
            this.fetchOrders();
          });
  ...
  fetchQuantityOrder(): void {
    this.http
      .get("http://localhost:8088/api/order-returns/quantity")
      .subscribe((res) => {
